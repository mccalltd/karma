...
var BaseReporter = function(formatError, reportSlow, adapter) {
  ...
  this.onBatchRunComplete = function(browsers, results) {
    if (browsers.length > 1 && !results.error && !results.disconnected) {
      if (!results.failed) {
        this.write(this.BATCH_SUCCESS, results.success);
      } else {
        this.write(this.BATCH_FAILED, results.failed, results.success);
      }
    }
  };
  ...
  this.BATCH_SUCCESS = 'BATCH: %d SUCCESS\n';
  this.BATCH_FAILED = 'BATCH: %d FAILED, %d SUCCESS\n';
};
