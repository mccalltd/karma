...
var start = function(injector, config, launcher, globalEmitter, preprocess, fileList, webServer,
  ...
  globalEmitter.on('browser_register', function(browser) {
    ...
    // captured).
    areAllCaptured = launcher.areAllCaptured();
    if (!runInBatches) && (config.autoWatch || config.singleRun) && areAllCaptured) {
      executor.schedule();
    }
    if (runInBatches && (launcher.getCapturedCount() % batchSize === 0 || areAllCaptured)) {
      executor.schedule();
    }
  });

  globalEmitter.on('batch_run_complete', function(browsers, results) {
    if (results.exitCode) {
      disconnectBrowsers(results.exitCode);
      return;
    }

    // `browser.launchId` links captured browser to launcher
    // `browser.id` links captured browser to socket
    var browserLaunchIds = [];
    var browserIds = [];
    browsers.forEach(function(browser) {
      browserLaunchIds.push(browser.launchId);
      browserIds.push(browser.id);
    });

    launcher.killBatch(browserLaunchIds, function() {
      removeDisconnectListeners(browserIds);
    });
  });
  ...
  var removeDisconnectListeners = function(keys) {
