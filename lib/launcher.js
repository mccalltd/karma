...
var Launcher = function(emitter, injector) {
  ...
  var kill = function(browsers, callback) {
    var remaining = 0;
    var finish = function() {
      remaining--;
  ...
  this.kill = function(callback) {
    ...
    kill(browsers, callback);
  };


  this.killBatch = function(ids, callback) {
    log.debug('Disconnecting browsers in batch');

    var browserBatch = browsers.filter(function(browser) {
      return ids.indexOf(browser.id) !== -1;
    });

    kill(browserBatch, callback);
  };
  ...
  this.getCapturedCount = function() {
    var count = 0;
    browsers.forEach(function(browser) {
